            ...
            if ($access === 'POST'){
                ...
                if ($status == 'R000'){
                    ...
                    $cart_id = $cart[1];;
          					...
          					$customer = $cart[3];
          					$total = $orderAmt/100;
                    $wechatpay = Module::getInstanceByName('wechatpay');
                    $wechatpay->validateOrder($cart_id, Configuration::get('PS_OS_PAYMENT'), $total, 'Wechat Pay', NULL, array(), $currency_id, false, $customer);
                }
                echo 'SUCCESS';
            }
            else{
